 ...
 */
  ...
  */
  ...
  constructor(private labelservice: LabelService, private archiveService: ArchiveService, private remainderService: RemainderService, public dialog: MatDialog, private router: Router, private notesService: NoteserviceService, private _cookieService: CookieService, private commonService: CommonService) {
    /**
  ...
  */
  displayMethod() {
    ...
    if (this.model.note != null && this.model.title != null && this.model.title != "") {
      ...
      */
      let obs = this.remainderService.noteData(this.model, this.email, this.PresentTime, this.color, this.isArchivedNote, this.labelname);
      obs.subscribe(
        (res: any) => {
          /**
...
*/
  ...
  */
  otherClearTimeDate(id) {
    ...
    let obs = this.remainderService.dateTimeChange(id, this.otherPresentTime, this.email);
    obs.subscribe(
      (res: any) => {
        this.notes = res;
      });
  ...
  * @return void
  * @param id 
  * @description Function to un archive note
  */
  unArchiveNote(id) {
    this.archive = !this.archive
   ...
   */
  deleteNote(id) {
    let obs = this.remainderService.deleteRemainderNotes(id, this.email);
    obs.subscribe(
      (res: any) => {
        if (res.error == 202) {
          alert("Unknown data");
